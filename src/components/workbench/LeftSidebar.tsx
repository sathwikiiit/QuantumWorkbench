...
import { 
  ...
  PlusCircle, Shield, Key, Zap, LayoutGrid, Layers
} from 'lucide-react';
import { 
...
export function LeftSidebar({ onAddTable }: LeftSidebarProps) {
  ...
  const schemas = useMemo(() => {
    const grouped: Record<string, typeof schema> = {};
    schema.forEach(table => {
      const name = table.schemaName || 'public';
    ...
    <div className="w-64 border-r bg-sidebar flex flex-col h-full shadow-inner">
      <div className="p-4 border-b space-y-4 bg-background/20">
        <div className="flex flex-col gap-3">
          <span className="text-[10px] font-black tracking-[0.2em] text-muted-foreground uppercase">Navigation</span>
          <nav className="space-y-1">
            <Link href="/">
              <div className={cn(
                "flex items-center gap-3 p-2 rounded-lg text-xs font-bold transition-all",
                isWorkbench ? "bg-primary/10 text-primary border border-primary/20" : "text-muted-foreground hover:bg-white/5"
              )}>
            ...
            <Link href="/connections">
              <div className={cn(
                "flex items-center gap-3 p-2 rounded-lg text-xs font-bold transition-all",
                pathname === '/connections' ? "bg-primary/10 text-primary border border-primary/20" : "text-muted-foreground hover:bg-white/5"
              )}>
            ...
            <Link href="/templates">
              <div className={cn(
                "flex items-center gap-3 p-2 rounded-lg text-xs font-bold transition-all",
                pathname === '/templates' ? "bg-primary/10 text-primary border border-primary/20" : "text-muted-foreground hover:bg-white/5"
              )}>
            ...
            <div className="p-3">
              <Accordion type="multiple" className="w-full" defaultValue={['public']}>
                {Object.entries(schemas).map(([schemaName, tables]) => (
                  <AccordionItem key={schemaName} value={schemaName} className="border-none">
